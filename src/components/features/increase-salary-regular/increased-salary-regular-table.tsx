...
import { getColumns } from './increased-salary-regular-table-column';
import React from 'react';
import { DataTableFilterField } from '@/types';
import { useTable } from '@/providers/table-provider';
import { getAllIncreasedSalaryRegular } from '@/db/queries/records';

export interface IncreaseSalaryRegularEligibleTableProps {
  listIncreasedSalaryRegular: ReturnType<typeof getAllIncreasedSalaryRegular>;
  cDepartment: any;
}

export default function IncreasedSalaryRegularTable({
  listIncreasedSalaryRegular,
  cDepartment,
}: IncreaseSalaryRegularEligibleTableProps) {
  const columns = getColumns();
  const { featureFlags } = useTable();
  const { data } = React.use(listIncreasedSalaryRegular);
