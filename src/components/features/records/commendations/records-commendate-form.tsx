...
import { createCommendations } from '@/db/actions/commendations';
import type { _getRecords } from '@/db/queries/records';
import { enumCommendationType } from '@/db/schema';
import {
  type CommendateRecordSchema,
  commendateRecordSchema,
} from '@/lib/zod/schemas/record-schema';

export interface RecordsCommendationFormProps {
  records: ReturnType<typeof _getRecords>;
}

export default function RecordsCommendationForm({
  records,
}: RecordsCommendationFormProps) {
  const form = useForm<CommendateRecordSchema>({
    resolver: zodResolver(commendateRecordSchema),
    defaultValues: {
      from: new Date(),
  ...
  const onSubmit = (values: CommendateRecordSchema) => {
    startCreateTransition(async () => {
      if (selected.length === 0 || selectedRecords.length === 0) {
        toast.error('Vui lòng chọn loại đánh giá và cán bộ');
      }
      const commendations = _.chain(selectedRecords)
        .groupBy('value')
        .map(item => {
          return {
            recordId: item[0].value,
            commendationType: item.map(ele => ele.id),
          };
        })
        .value();

      const mappedValue = commendations.map(item => {
        return {
          ...values,
          recordId: item.recordId,
          commendationType: item.commendationType,
        };
      });
      const { error } = await createCommendations(mappedValue);
      if (!error) {
        ...
        >
          ...
          <CollapsibleContent className="CollapsibleContent overflow-visible px-5">
            <FancyMultiCombobox
              ...
              dataset={enumCommendationType.map(ele => ({
                value: ele,
                label: ele,
              }))}
            />
