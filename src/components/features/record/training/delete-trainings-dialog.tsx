...
interface DeleteTrainingsDialogProps
  extends React.ComponentPropsWithoutRef<typeof Dialog> {
  trainings: Row<RecordsTraining>['original'][];
  showTrigger?: boolean;
  onSuccess?: () => void;
  name: string;
}

export function DeleteTrainingsDialog({
  trainings,
  showTrigger = true,
  onSuccess,
  name,
  ...props
}: DeleteTrainingsDialogProps) {
  const [isDeletePending, startDeleteTransition] = React.useTransition();
  const isDesktop = useMediaQuery('(min-width: 640px)');

  function onDelete() {
    startDeleteTransition(async () => {
      const { error } = await deleteTrainings({
        ids: trainings.map(training => training.id),
      });
      if (error) {
        toast.error(error);
        return;
