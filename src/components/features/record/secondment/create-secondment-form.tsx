...
import { createSecondment } from '@/db/actions/secondments';
...
export interface RecordsSecondmentFormProps {
  records: ReturnType<typeof _getRecords>;
  departments: ReturnType<typeof getAllDepartments>;
  // secondments: ReturnType<typeof getAllSecondments>;
  duties: ReturnType<typeof getAllDuties>;
  onSuccess: () => void;
  recordId: string;
}

export default function CreateSecondmentForm({
  records,
  departments,
  duties,
  onSuccess,
  ...props
}: RecordsSecondmentFormProps) {
  const form = useForm<CreateSecondmentSchema>({
    resolver: zodResolver(createSecondmentSchema),
    defaultValues: {},
  });
  // const { data } = use(records);
  const resultDepartments = use(departments);
  const resultDuties = use(duties);
  // const { data: dataFormSecondments } = use(secondments);
  ...
  const onSubmit = (values: CreateSecondmentSchema) => {
    startCreateTransition(async () => {
      // console.log(values);
      const { error } = await createSecondment({
        ...values,
