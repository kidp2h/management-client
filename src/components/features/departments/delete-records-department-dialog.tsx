...
interface DeleteDepartmentsDialogProps
  ...
  recordsDepartment: Row<Departments>['original'][];
  showTrigger?: boolean;
  onSuccess?: () => void;
  name: string;
}

export function DeleteRecordsDepartmentDialog({
  recordsDepartment,
  showTrigger = true,
  onSuccess,
  name,
  ...
  console.log(recordsDepartment);
  function onDelete() {
    startDeleteTransition(async () => {
      const { error } = await deleteRecordsDepartment(
        recordsDepartment.map(i => i.id),
      );
      if (error) {
        toast.error(error);
        return;
      ...
      toast.success('Thành viên trong đơn vị đã được xoá');
      onSuccess?.();
    });
  }
  if (isDesktop) {
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <span className="font-medium">{recordsDepartment.length} </span>
              {name}.
            </DialogDescription>
          </DialogHeader>
